...
#=
Here I avoids direct representation πf(x) by evaluating f(x, y) only at nodal points.
For plotting use linear interpolation c:
=#

f(x, y) = 10 * sin(x) * sin(y) * exp(-(x^2 + y^2)^0.5)
mesh = TriangulationOfRect([-5, -5], [5, 5], [25, 25])
fᵢ = Vector{Float64}(undef, size(mesh.P)[2])
for (i, point) in enumerate(eachcol(mesh.P))
    x, y = point[1], point[2]
    println(x, '\t', y)
    fᵢ[i] = f(x, y)
end

for (point, val) in zip(eachcol(mesh.P), fᵢ)
    println(join([point..., val], '\t'))
end
